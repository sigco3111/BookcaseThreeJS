...
const bloomPass = new UnrealBloomPass(
  new THREE.Vector2(window.innerWidth, window.innerHeight), 0.35, 0.55, 1.0
);
...
const fDim = gui.addFolder('Dimensions');
fDim.add(params, 'width', 0.7, 3.2, 0.01).name('width (m)').onChange(rebuild);
fDim.add(params, 'height', 1.2, 3.0, 0.01).name('height (m)').onChange(rebuild);
fDim.add(params, 'depth', 0.24, 0.6, 0.005).name('depth (m)').onChange(rebuild);
...
fCon.add(params, 'crown').name('crown molding').onChange(rebuild);
fCon.add(params, 'faceFrame').name('face frame').onChange(rebuild);
fCon.add(params, 'sidePanels').name('side insets').onChange(rebuild);
fCon.add(params, 'back', ['planks', 'flat', 'open']).name('back style').onChange(rebuild);
fCon.close();

const fFinish = gui.addFolder('Finish');
fFinish.addColor(style, 'tint').onChange((v) => wood.color.set(v));
fFinish.add(style, 'roughness', 0.4, 1.5, 0.01).onChange((v) => { wood.roughness = v; });
fFinish.add(params, 'grainScale', 0.5, 2.5, 0.01).name('grain scale').onChange(rebuild);
fFinish.add(params, 'grainFlip').name('grain direction').onChange(rebuild);
fFinish.close();
...
fStudio.add(studioParams, 'bloom', 0, 1.5, 0.01).onChange((v) => { bloomPass.strength = v; });
fStudio.add(studioParams, 'vignette', 0, 1, 0.01).onChange((v) => { gradePass.uniforms.uVignette.value = v; });
fStudio.add(studioParams, 'grain', 0, 0.15, 0.001).onChange((v) => { gradePass.uniforms.uGrain.value = v; });
